...
from utils.local_search import LocalSearch
...
local_search_iter = 20 # 个体局部搜索深度
tourment_select_size = 20 # 局部搜索锦标赛大小
pls = 0.1 # 局部搜索概率

testdir = "./MK"
resultdir = "./result/nsgals/chrome"
...
def NSGALS(data: str = './test/None', result: str = './result/None') -> np.ndarray:
    ...
    zmax = np.max(popfun, axis=0).reshape(1, mobj)
    # 创建进化算子
    ga = EvolutionOperator(pc, pm)
    # 创建局部搜索算子
    ls = LocalSearch(local_search_iter, tourment_select_size, pls, mobj)
    for i in range(iter_max):
        ...
        zmin, zmax = GetMaxAndMinObj(offfun, zmin, zmax)
        # 局部搜索
        ls.LocalSearch(refer_points, population, zmin, zmax)
        # 更新理想点
        offfun = population.CalChildfunc(mobj)
        zmin, zmax = GetMaxAndMinObj(offfun, zmin, zmax)
        # 得到融合种群目标函数
    ...
    # if mobj <= 3:
    #     popfun = population.popfun
    #     fig, ax = plt.subplots(1, 1, figsize=(10, 8), subplot_kw={'projection': '3d'})
    #     ax.scatter3D(popfun[:, 0], popfun[:, 1], popfun[:, 2], c='r', s=10)
    #     plt.pause(100)
    return popfun
